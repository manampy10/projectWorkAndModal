...
document.addEventListener("DOMContentLoaded", () => {
  const authToken = getToken(); // Vérifier si l'utilisateur est connecté
  ...
  //Après que je click sur ajouter une photo le formulaire s'affiche dans le modale
  ...
  backButton.addEventListener("click", () => {
    ...
    //Remet le titre en "Galerie Photo"
  ...
  modalGallery.addEventListener("click", async function (event) {
    if (event.target.classList.contains("delete-btn")) {
      ...
      deleteWorkFromModal(photoId, photoItem);
    }
  });
});

// Affiche le modal quand on click sur modifier
export function setupModal(authToken) {
  ...
  if (authToken && editProjectsBtn) {
    editProjectsBtn.addEventListener("click", (event) => {
      ...
      //Cela évite d'afficher deux fois les mêmes projets quand on clique plusieurs fois sur "Modifier".
      modalGallery.innerHTML = "";
      //je recharge la galerie avec loadWorksInModal() pour afficher les projets actuels dans la fenêtre modale.
      loadWorksInModal();
      //On cache le formulaire d’ajout de photo, pour afficher uniquement la galerie au début.
      addPhotoForm.style.display = "none";
      //On cache le bouton de retour ← (on ne l’utilise que quand on est dans le formulaire).
      backButton.style.display = "none";
      // C’est ce bouton qui permettra d’afficher le formulaire ensuite.
      addPhotoBtn.style.display = "block";
    });
  }
