...
export async function loadWorks() {
  ...
  try {
    ...
    // Création d'un ensemble avec les noms des catégories, On évite d'afficher plusieurs fois le même bouton de filtre.
...
function generateFilters(categories, works, categoriesData) {
  ...
  categories.forEach((category) => {
    ...
    button.addEventListener("click", () => {
      ...
      //Si l’utilisateur clique sur "Tous", on met categoryId = null pour ne pas filtrer.
      const categoryId =
        category === "Tous"
          ? null
...
export async function deleteWorkFromModal(workId, element) {
  ...
  try {
    ...
    works.forEach((work) => {
      ...
      deleteBtn.addEventListener("click", () =>
        deleteWorkFromModal(work.id, photoDiv)
      );
...
// Ajouter un nouveau projet (par exemple, quand l'utilisateur connecté ajoute une image)
